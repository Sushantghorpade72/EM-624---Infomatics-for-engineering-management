...
# opening the files
Pos_file = open("pro_ marijuana_raw.txt","r")
Con_file = open("con_ marijuana_raw.txt","r")
...
for s_word in stopwords_file:
    stpwords.append( s_word.strip() )
stpwords.extend(['marijuana', 'use', 'legal', 'year', 'people', 'legalization'])
...
for text_file1 in Pos_lines:
    ...
    for word in parts:
        ...
        if word.lower() not in stpwords and len(word) > 2:
            Pos_words.append(word.lower())

# Processing the "against" lines
